...
# Load the same MR baseline solver used by R190.
...
src=(INP/'solver_core.py').read_text(); tmp=OUT/'_r194_mr_baseline.py'; tmp.write_text(src.replace(OLD,MR))
sp=importlib.util.spec_from_file_location('r194_mr_baseline',tmp); m=importlib.util.module_from_spec(sp); assert sp.loader; sp.loader.exec_module(m)
K=10000.; EPS=.05; GAM=.2; BETA=np.array([1000.,3000.,3000.]); NR=34; NZ=20; EDGE_CELLS=10
...
def smooth_inner_weight(r,center,halfwidth):
    r=np.asarray(r,float)/float(m.R); c=float(center); d=float(halfwidth); lo,hi=c-d,c+d
    t=np.clip((r-lo)/(hi-lo),0.,1.); s=6*t**5-15*t**4+10*t**3; w=1-s
    return np.where(r<=lo,1.,np.where(r>=hi,0.,w))

def partition_source_energy(z,N,DG,Rq,W,mask):
    ...
    I1b=Js**(-2/3)*I1; I2b=Js**(-4/3)*I2
    # dW/df at fixed total shear-modulus normalization.
    return jnp.sum(mask*0.5*(I2b-I1b)*W)
part_grad=jax.jit(jax.vmap(jax.grad(partition_source_energy),in_axes=(0,0,0,0,0,0)))

def assemble_partition_source(mesh,y,mask,is_pre):
    lv=m.local_vars(mesh,y,0.,EPS) if is_pre else m.local_vars(mesh,y,GAM,EPS)
    gl=np.asarray(part_grad(jnp.asarray(lv),jnp.asarray(mesh['N']),jnp.asarray(mesh['G']),jnp.asarray(mesh['Rq']),jnp.asarray(mesh['W']),jnp.asarray(mask)))
    gg=np.zeros(len(y))
...
def beta_residual_derivative(mesh,y,gamma,channel):
    ...
    for ids,smaps,Ns,rr,wt,tap,zidx in mesh['surf']:
        ...
        scales=np.array([1.,rr,1.]); kk=kbase[c]*wt*tap*scales[c]**2; smap=smaps[c]; active=np.flatnonzero(smap>=0); gm=smap[active]; Na=Ns[active]
        np.add.at(gg,gm,kk*delta*Na)
    return gg

def top_stress_partition_weighted(mesh,y,gamma,c,center,halfwidth):
    ...
    for i in range(mesh['nr']):
        ...
        for xi in m.GP3:
            ...
            P=post.piso(F,post.F_MR+c*w,0.,0.,0.)+pq*np.linalg.inv(F).T; sig=(P@F.T)/J*G0*1000.; out['r_cur'].append(rr); out['szz_Pa'].append(sig[2,2])
    return {k:np.asarray(v) for k,v in out.items()}

def raw_prof(mesh,ybp,ysp,c=0.,center=None,halfwidth=.02):
    p0=top_stress_partition_weighted(mesh,ybp,0.,c,center,halfwidth); pg=top_stress_partition_weighted(mesh,ysp,GAM,c,center,halfwidth); coords,_=m.unpack(mesh,ysp,GAM,EPS); top=np.isclose(mesh['X'][:,1],mesh['H']); a=float(np.max(coords[top,0])); o=np.argsort(p0['r_cur']); bp=np.interp(pg['r_cur'],p0['r_cur'][o],p0['szz_Pa'][o]); return pg['r_cur']/a,-(pg['szz_Pa']-bp),a
