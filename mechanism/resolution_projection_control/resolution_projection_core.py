...
def load_mr():
    src=(INP/'solver_core.py').read_text(); tmp=OUT/'_r190_mr.py'; tmp.write_text(src.replace(OLD,MR))
    sp=importlib.util.spec_from_file_location('r190_mr',tmp); m=importlib.util.module_from_spec(sp); assert sp.loader; sp.loader.exec_module(m); return m
m=load_mr()
